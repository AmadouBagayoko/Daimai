...
public interface DepenseRepository extends JpaRepository<Depense, Long> {
    ...
    // Projection DTO pour éviter les requêtes N+1
    @Query("SELECT new groupe_9_daimai.com.Daimai.DTO.DepenseResponseDTO(d.id, d.montant, d.categorie, d.dateDepense, d.association.id, d.association.nom) FROM Depense d WHERE d.id = :id")
    DepenseResponseDTO findDepenseDTOById(@Param("id") Long id);

    @Query("SELECT new groupe_9_daimai.com.Daimai.DTO.DepenseResponseDTO(d.id, d.montant, d.categorie, d.dateDepense, d.association.id, d.association.nom) FROM Depense d")
    List<DepenseResponseDTO> findAllDepenseDTOs();

    @Query("SELECT new groupe_9_daimai.com.Daimai.DTO.DepenseResponseDTO(d.id, d.montant, d.categorie, d.dateDepense, d.association.id, d.association.nom) FROM Depense d WHERE d.association.id = :associationId")
